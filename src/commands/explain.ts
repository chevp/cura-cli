...
const HELP = `cura explain — ask the active LLM provider to diagnose the most recent
cura ship/commit failure (read-only, never executes anything).

Usage:
  cura explain                  # diagnose the last logged failure
  cura explain "<question>"     # ad-hoc question with current git state
  cura explain --show           # print the raw error log, do not call the LLM
  cura explain --clear          # delete the error log

Where the log lives:
  <repo>/.git/cura-last-error.log   (one per repo)

Provider is local Ollama (CURA_OLLAMA_HOST/MODEL).
Run 'cura doctor ollama' to verify it is reachable.
`;
...
export async function run(argv: string[]): Promise<number> {
  ...
  for (const a of argv) {
    ...
    if (a === "--show") {
      ...
      process.stderr.write(`cura explain: unknown option '${a}'\n`);
      return 1;
    } else {
      if (question) {
        process.stderr.write("cura explain: only one free-form question accepted\n");
        return 1;
      }
      question = a;
    }
  ...
  if (!isInsideRepo()) {
    process.stderr.write("cura explain: not a git repository\n");
    return 1;
  }
  const dir = gitDir();
  if (!dir) return 1;
  const log = join(dir, "cura-last-error.log");

  if (mode === "clear") {
    if (existsSync(log)) {
      try {
        rmSync(log, { force: true });
        process.stdout.write(`cura explain: cleared ${log}\n`);
      } catch (err) {
        process.stderr.write(`cura explain: failed to clear ${log}: ${String(err)}\n`);
        return 1;
      }
    } else {
      process.stdout.write("cura explain: no error log to clear\n");
    }
    return 0;
  }
  ...
  if (mode === "show") {
    if (!existsSync(log)) {
      process.stderr.write(`cura explain: no error log at ${log}\n`);
      return 1;
    }
  ...
  if (existsSync(log)) {
    ...
    if (!question) {
      process.stderr.write(
        `cura explain: no error log at ${log}\n\n` +
          "There has been no failed cura ship/commit recorded in this repo. Either:\n" +
          "  - run 'cura ship' / 'cura commit --push' until something fails, or\n" +
          '  - pass a free-form question:  cura explain "why is git push hanging?"\n',
      );
      return 1;
    }
  ...
  const max = Number.parseInt(process.env.CURA_EXPLAIN_MAX_CHARS ?? "6000", 10) || 6000;
  ...
  if (!(await provider.ping())) {
    process.stderr.write(
      `cura explain: provider '${activeProviderName()}' not reachable\n` +
  ...
  try {
    ...
    process.stderr.write(`cura explain: provider '${activeProviderName()}' request failed\n`);
    return 1;
  }

  if (!answer.trim()) {
    process.stderr.write("cura explain: provider returned empty response\n");
    return 1;
  }
