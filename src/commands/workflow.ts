...
const HELP = `cura workflow — manage and run scripted workflows from .che/workflows/*.yml

Usage: cura workflow <subcommand> [args]

Subcommands:
  list                       list workflows in the current repo
  show <name>                print the parsed plan of a workflow
  run  <name> [--k=v ...]    execute a workflow (also: cura run <name>)
...
function cmdList(): number {
  ...
  if (!where) {
    process.stderr.write(`cura workflow: no .che/workflows/ found above ${process.cwd()}\n`);
    return 1;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (Array.isArray(t)) {
        trig = t.filter((v) => typeof v === "string").join(" | cura ");
...
function cmdShow(argv: string[]): number {
  if (argv.length === 0 || argv[0] === "-h" || argv[0] === "--help") {
    process.stdout.write(`Usage: cura workflow show <name>\n`);
    return argv.length === 0 ? 1 : 0;
  }
  const name = argv[0]!;
  if (argv.length > 1) {
    process.stderr.write("cura workflow show: unexpected extra arguments\n");
    return 1;
  }
  let resolved;
  try {
    ...
    process.stderr.write(`cura workflow: ${(err as Error).message}\n`);
    return 1;
  }
  ...
  const t = getPath(doc, ".trigger");
  if (Array.isArray(t)) {
    const list = t.filter((v) => typeof v === "string").join(" | cura ");
    process.stdout.write(`${c.bold("trigger")}     cura ${list}\n`);
  } else if (typeof t === "string") {
    process.stdout.write(`${c.bold("trigger")}     cura ${t}\n`);
  }
...
async function cmdRun(argv: string[]): Promise<number> {
  ...
  if ("help" in parsed) {
    process.stdout.write(
      `cura workflow run — execute a workflow's steps.

Usage:
  cura workflow run <name> [--key=value ...]
  cura run <name> [--key=value ...]      (alias)
...
`,
  ...
  if ("error" in parsed) {
    process.stderr.write(`cura workflow: ${parsed.error}\n`);
    return 1;
  }
  if (!parsed.name) {
    process.stderr.write("cura workflow run: missing workflow name\n");
    return 1;
  }
  ...
  if (missing.length > 0) {
    process.stderr.write(`cura workflow: missing required input(s): ${missing.join(" ")}\n`);
  ...
  // Per-repo workflows expect to run from their root (script paths are
  // relative to .che/).
  if (!resolved.builtin) {
...
export async function run(argv: string[]): Promise<number> {
  ...
  switch (sub) {
    ...
    case "--help":
      ...
      return sub === "" ? 1 : 0;
    default:
      process.stderr.write(`cura workflow: unknown subcommand '${sub}'\n`);
      process.stderr.write(HELP);
      return 1;
  }
}

/** Public entry for the top-level `cura run <name>` alias. */
